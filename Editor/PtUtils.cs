...
public static class PtUtils {
  ...
  private const string MANIFEST_FILE_NAME = "poly_toolkit_manifest";
  /// <summary>
  ...
  public static string GetPtBaseLocalPath() {
      ...
      string[] matches = AssetDatabase.FindAssets(MANIFEST_FILE_NAME);
      ...
      string manifestFilePath = AssetDatabase.GUIDToAssetPath(matches[0]);

      string manifestPath = manifestFilePath.Replace('\\', '/').Replace(string.Format("{0}.dat", MANIFEST_FILE_NAME), "");

      basePath = NormalizeLocalPath(manifestPath);
      return basePath;
    }
